...
select tbl_cursos.nome as curso, tbl_disciplina.nome as disciplina from tbl_cursos_disciplina
inner join tbl_cursos on tbl_cursos.id=tbl_cursos_disciplina.curso_id
inner join tbl_disciplina on tbl_disciplina.id=tbl_cursos_disciplina.disciplina_id
order by tbl_cursos.id;
...
select tbl_redacao.id, tbl_redacao.titulo, tbl_redacao.texto, tbl_tema.nome as tema, tbl_redacao.status 
                    from tbl_redacao 
                    inner join tbl_tema on tbl_redacao.tema_id=tbl_tema.id  
                    where tbl_redacao.aluno_id=5;
                    
insert into tbl_redacao (titulo, texto, tema_id, aluno_id, status)values(
	'',
	'No filme WALL-E, do estúdio Pixar, somos apresentados a um planeta Terra poluído e inabitável. A trama demonstra como práticas sustentáveis poderiam ter evitado essa situação. Infelizmente, essa realidade não está distante, considerando a ausência de práticas ecológicas e a poluição gerada por veículos automotores. Por isso, é urgente analisar os fatores que colaboram para esse cenário.
    Convém ressaltar que o problema advém, em parte, da falta de práticas sustentáveis. A expansão "Vida Sustentável" (2020), do jogo The Sims 4, por exemplo, incentiva jogadores a adotarem atitudes ecológicas, como a substituição de energia elétrica por energia solar e o cultivo de alimentos. No jogo, práticas sustentáveis reduzem impostos, enquanto a ausência dessas medidas resulta em punições. Esse tipo de interação lúdica permite que os jogadores experimentem, em ambiente virtual, soluções que podem ser aplicadas na vida real, mostrando a importância da sustentabilidade.
    Além disso, a poluição causada por veículos automotores agrava esse cenário. De acordo com o Instituto de Energia e Meio Ambiente (IEMA) (2019), sete poluentes prejudiciais à saúde são regulados no Brasil, exigindo monitoramento pelo poder público. Segundo a pesquisadora Beatriz Sayuri Oyama, do IEMA, a redução da poluição em centros urbanos é um desafio, especialmente em relação a poluentes de formação complexa, como o material particulado e o ozônio. Logo, medidas como a redução da frota e o incentivo a transportes alternativos são cruciais para melhorar a qualidade do ar.
    Portanto, é evidente que políticas ambientais eficazes são essenciais para o desenvolvimento sustentável. Cabe aos governos estaduais e municipais adotarem medidas mais rigorosas, como multas e regulamentações, além de promoverem práticas ecológicas nas cidades. Ademais, é fundamental implementar uma educação sustentável nas escolas, para que os alunos aprendam a reciclar, reutilizar e reduzir, contribuindo para combater a degradação do planeta. Assim, espera-se evitar o futuro apresentado em WALL-E.',
	3,
	5,
true
);
